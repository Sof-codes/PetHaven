...
/**
 * MANAGE PETS (ADMIN)
 * Lists all pets in the system with options to delete or view details.
...
if(isset($_GET['delete'])) {
    $name = $_GET['delete'];
    $pdo->prepare("DELETE FROM pets WHERE name = ?")->execute([$name]);
    header("Location: pets_list.php?msg=deleted");
}

$pets = $pdo->query("SELECT p.*, c.name as category_name FROM pets p LEFT JOIN categories c ON p.category_name = c.name ORDER BY p.created_at DESC")->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .action-btn { padding: 8px 12px; border-radius: 6px; font-size: 0.85rem; margin-right: 5px; text-decoration: none; display: inline-block; }
    </style>
</head>
...
<div class="admin-content">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        ...
        <a href="add_pet.php" class="btn btn-primary"><i class="fa-solid fa-plus-circle"></i> Add New Pet</a>
    </div>
    ...
    <table class="pet-table">
        ...
        <tbody>
            ...
            <tr>
                <td><div style="width: 50px; height: 50px; background: #eee; border-radius: 8px; overflow: hidden;"><img src="../<?php echo $pet['image']; ?>" style="width: 100%; height: 100%; object-fit: cover;"></div></td>
                ...
                <td>
                    <span class="tag" style="background: <?php echo $pet['status'] == 'available' ? '#d4edda' : '#f8d7da'; ?>; color: <?php echo $pet['status'] == 'available' ? '#155724' : '#721c24'; ?>;">
                        <?php echo ucfirst($pet['status']); ?>
                    </span>
                </td>
                <td>
                    <div style="display: flex; gap: 5px;">
                        <a href="../pet_details.php?name=<?php echo urlencode($pet['name']); ?>" class="action-btn" style="background: var(--color-secondary); color: black;"><i class="fa-solid fa-eye"></i></a>
                        <a href="?delete=<?php echo urlencode($pet['name']); ?>" class="action-btn" style="background: var(--color-danger); color: white;" onclick="return confirm('Are you sure you want to delete this pet profile?');"><i class="fa-solid fa-trash-can"></i></a>
                    </div>
                </td>
            </tr>
            <?php endforeach; ?>
